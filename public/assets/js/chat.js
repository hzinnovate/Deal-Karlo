...
window.addEventListener('load', () => {
    if (profile === undefined || profile === null || profile === 'undefined' || profile === 'null') {
        window.location = '../index.html';
    }
});
firebase.database().ref(`chat/${profile.uid}`).on('value', (e) => {
    let chatKeys = [];
    let frnds = e.val();
    if (frnds !== null) {
        ...
        firebase.database().ref(`userProfiles`).once('value', (e) => {
            ...
            let all = e.val();
            for (var key in all) {
                if (chatKeys.indexOf(key) !== -1 && key !== profile.uid) {
                    document.getElementById('contact').innerHTML += `
...
function chat(ths) {
    ...
    firebase.database().ref(`userProfiles/${key}`).once('value', (e) => {
        ...
        document.getElementById('chatHeader').innerHTML +=
            ...
            <div class="user_info">
            <span style="color: black; font-weight: bolder;">${head.name}</span>
            </div>
            `;
        document.getElementById('mbChatHeader').innerHTML +=
            `
    ...
    <div class="user_info">
    <span style="color: black !important; font-weight: bolder;">${head.name}</span>
    </div>
    `;
            ...
            if (token.val() == uid) { //Getting the token of the reciever using  if condition..!   
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({
                        "to": token.key,
                        "notification": {
                            "title": `${name} send you a messege on chat`,
                            "body": msg,
